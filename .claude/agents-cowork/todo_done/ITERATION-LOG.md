...
- SessionList decomposition refactor -- SessionList.tsx reduced from 708 to 532 lines (25% reduction); extracted useSessionListActions.ts hook (238 lines) containing all session CRUD operations (load/open/delete/fork/export/rename/commitRename), global search logic (handleGlobalSearch/handleOpenGlobalResult), multi-select mode (selectMode/selectedIds/exitSelectMode/toggleSelectId/bulkDelete), and deleteAll; fixed anti-pattern runtime require() with proper useSessionStore import; pure refactor with zero visual or behavioral changes; build SUCCESS New IPC handler `skills:fetchClawhub` in main process uses Electron net.fetch to call ClawhHub REST API (`/api/v1/skills?sort=downloads&limit=50`) with CORS-free main-process networking; tries alternate endpoint on empty response; returns transformed skill data or graceful error; (2) Preload bridge `skillsFetchClawhub()` added; (3) SkillsPanel.tsx enhanced with remote skill state management: `remoteSkills`, `fetchingRemote`, `remoteFetchResult` state; `fetchClawhubSkills` callback that transforms API response into MarketplaceSkill format and deduplicates against static catalog; `allMarketplaceSkills` useMemo combines static + remote; all filter pill counts updated to use combined list; (4) New "Fetch from ClawhHub" button with CloudDownload icon, loading spinner animation, accent-colored hover, and status indicator showing results count or error message; (5) 6 new i18n keys in en.json and zh-CN.json (fetchFromClawhub, fetchingClawhub, newSkillsFound, noNewSkills, clawhubEmpty, clawhubError); addresses user feedback for dynamic skill loading from ClawhHub marketplace; 4 files changed; build SUCCESS
