...
const BASH_TOOLS = new Set(['Bash', 'computer', 'PowerShell'])
...
function parseCronJobs(raw: string): CronJob[] | null {
  try {
...
function WebBrowserInputCard({ input }: { input: Record<string, unknown> }) {
...
function WebBrowserResultCard({ result, status, t }: { result: string; status: string | undefined; t: (k: string) => string }) {
  const [expanded, setExpanded] = useState(false)
