...
// ── UI store ────────────────────────────────────
interface UiState {
  sidebarTab: 'history' | 'files' | 'settings' | 'notes' | 'skills' | 'memory'
  sidebarOpen: boolean
  terminalOpen: boolean
  commandPaletteOpen: boolean
  toasts: ToastItem[]
  setSidebarTab: (tab: 'history' | 'files' | 'settings' | 'notes' | 'skills' | 'memory') => void
  ...
  activeNavItem: 'chat' | 'history' | 'files' | 'terminal' | 'settings' | 'notes' | 'skills' | 'memory'
...
export const useUiStore = create<UiState>((set) => ({
  ...
  setActiveNavItem: (item) => set((s) => {
    if (item === 'history' || item === 'files' || item === 'settings' || item === 'notes' || item === 'skills' || item === 'memory') {
