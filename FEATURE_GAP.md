...
- `system.init.mcp_servers[]`：MCP 服务器连接状态，AIPA 未消费
...
- ⚠️ `mcp:getTools` — 已有 IPC 但返回空数组（注释：`Real tool enumeration requires MCP protocol integration`）
- ⚠️ `mcp:reconnect` — 已有 IPC 但返回 `error: 'Reconnect not supported'`
- ❌ MCP 服务器连接状态实时展示（无 WebSocket/SSE 连接状态监控）
...
P1：`mcp:getTools` 实现真实工具枚举（需建立 MCP 客户端连接，或解析 CLI `system.init.mcp_servers` 事件）。
P1：MCP 服务器连接状态实时展示（connected/disconnected/error）。
P2：`.mcp.json` 项目级管理。
