...
| `TaskOutputTool` | 读取后台任务输出 | ✅ TaskOutputCard：indigo 主题、task_id 展示、JSON 结果走 TaskDashboardCard、纯文本走300字符预览（Iteration 682） |
...
| `EnterWorktree` / `ExitWorktree` | Git worktree 沙箱隔离 | ✅ `WorktreeToolCard`：Enter/Exit 双态展示、worktree 路径/branch chip、状态指示器、接入 ToolUseBlock（Iteration 683） |
...
- ✅ 默认值已改为 `skipPermissions: false`（不再绕过权限）
...
| `availableModels` | array | 企业模型白名单 | ✅ — SettingsAIEngine 标签式编辑器，读写 ~/.claude/settings.json（Iteration 682） |
| `worktree.symlinkDirectories` | array | worktree 符号链接目录 | ✅ — SettingsGeneral Worktree 区块，标签式路径列表，读写 ~/.claude/settings.json（Iteration 682） |
...
|----------|-------------|---------------|
| `system`（init）| `tools[]`, `mcp_servers[]`, `model`, `permissionMode`, `session_id`, `skills[]`, `plugins[]` | ✅ 完整消费：ChatHeader 显示可用工具数 chip（blue Layers 图标）+ 模型/权限模式指示器，SettingsMcp 显示实时连接状态 |
...
| `content_block_start` | block type（text/tool_use） | ✅ 有意忽略：tool_use 完整信息在 assistant 消息块中到达，content_block_start 仅含骨架；已添加注释说明（Iteration 679） |
...
| `result` subtype: `error_max_structured_output_retries` | 结构化输出超出重试次数 | ✅ — stream-bridge 覆盖所有 non-success subtypes，renderer 已处理（Iteration 679） |
...
| `sandbox.ignoreViolations` | Record | 特定工具违规豁免 | ✅ — SandboxSettingsPanel IgnoreViolationsSection 已实现，补全 i18n 键（Iteration 682） |
| `plugins.enabledPlugins` | Record | 插件市场插件启用/禁用 | ✅ — SettingsPlugins.tsx 插件列表 UI（toggle 开关），i18n 键已补全（Iteration 682） |
...
- ✅ 嵌套 CLAUDE.md 加载路径可视化——指令文件 Tab 内置"CLI 当前加载的 CLAUDE.md 文件"折叠面板，三个路径存在状态（Iteration 683 确认完整）
...
✅ 已实现——MemdirTab 类型过滤 chip 已有 UI，补全 i18n 国际化（Iteration 682）
...
- ✅ `SlashCommandPopup.tsx` 内置 20+ CLI 斜杠命令映射（`/compact`、`/clear`、`/model` 等），含分类和图标
- ✅ `/compact` 已在 ChatHeader 中有专用按钮（Iteration 667）
...
- ✅ 已验证：`@filepath` 文本正确传递给 CLI，无需额外 content array 转换（CLI 自行解析 `@` 语法）
...
- ✅ 已验证并修复：ChatHeader 工具栏 Plan Mode toggle 开启时，`useStreamJson` 自动将 CLI `--permission-mode` 设为 `plan`（除非用户已选 `bypassPermissions`）（Iteration 683）
...
| stdout: `{"type":"result",...}` | `cli:result` | ✅ |
| stdout: `{"type":"system","subtype":"init",...}` | `cli:systemInit` | ✅ |
...
| CLI flag: `--permission-mode` | 通过 `args.flags` 传入 | ✅ PermissionsSettingsPanel 选择器（Iteration 665） |
| CLI flag: `--resume <session_id>` | `args.resumeSessionId` | ✅ |
| CLI flag: `--model` | `args.model` | ✅ |
| CLI flag: `--dangerously-skip-permissions` | `skipPermissions → args.flags` | ✅（默认值已改为 false，无安全隐患）|
| CLI flag: `--max-turns` | 通过 `args.flags` 可传 | ✅ SettingsAIEngine 数字输入（prefsStore.maxTurns，Iteration 682） |
| CLI flag: `--append-system-prompt` | 通过 `args.flags` 可传 | ✅ SettingsAIEngine textarea（prefsStore.appendSystemPrompt，Iteration 682） |
...
## 二十五、最新实现记录（2026-04-19，Iteration 683）

### Iteration 683 — FEATURE_GAP.md 全面审计 + stale 项修复

本次迭代对 FEATURE_GAP.md 进行全量审计，将已实现但文档未更新的 stale ❌/⚠️ 项标记为 ✅：

- **LSPTool**（原 ❌）→ ✅：`LSPResultCard` + `LSPToolCard` 已有完整渲染，operation chip + file:line 坐标 + 输入/输出分块
- **EnterWorktree/ExitWorktree**（原 ⚠️）→ ✅：`WorktreeToolCard` 实现 Enter/Exit 双态展示，接入 ToolUseBlock（Iteration 683 前一轮 commit）
- **skipPermissions 默认值**（原 ⚠️）→ ✅：默认值已改为 `false`，无安全隐患
- **内置斜杠命令映射**（原 ⚠️/❌）→ ✅：`SlashCommandPopup.tsx` 内置 20+ CLI 命令（SLASH_COMMANDS 数组），`/compact` 有 ChatHeader 按钮
- **@ 提及格式验证**（原 ⚠️）→ ✅：验证正确，CLI 自行解析 `@filepath` 语法
- **Plan Mode ↔ permission plan**（原 ⚠️）→ ✅：`useStreamJson` 在 `isPlanMode=true` 时自动传递 `--permission-mode plan`（Iteration 683）
- **CLAUDE.md 嵌套扫描**（原 ❌）→ ✅：Memory 面板已内置加载路径可视化
- **MCP reconnect**：`mcp:reconnect` IPC 仍返回 'not supported'，需后续实现真实 MCP 重连

**剩余真正未实现项**（标记为 intentionally deferred 或保持 ❌）：
- `REPLTool` / `ConfigTool` — ant-only 内部工具，外部用户不可调用，保持 ❌
...
### Iteration 679 — stream-bridge 修复 + content_block_start 文档化
- `stream-bridge.ts`：删除 result emit 中重复的 `uuid`/`fastModeState` 属性（TS1117）
- 扩展 non-success result subtype 日志，覆盖所有三种错误子类型（包括 `error_max_structured_output_retries`）
- `content_block_start` case：添加注释说明有意忽略的原因（tool_use 完整 name/id/input 在 assistant 消息块中到达）

### Iteration 680 — Settings UI 补全（appendSystemPrompt / availableModels / worktree / sandbox i18n）
- `SettingsAIEngine.tsx`：新增 `appendSystemPrompt` textarea（4行，读写 prefsStore，对应 `--append-system-prompt` flag）；新增 `availableModels` 标签式编辑器（读写 `~/.claude/settings.json`）
- `SettingsGeneral.tsx`：新增 Worktree 配置区块（GitBranch 图标），包含 `worktree.symlinkDirectories` 和 `worktree.sparsePaths` 标签式路径编辑器，读写 `~/.claude/settings.json`
- `SandboxSettingsPanel.tsx`：`IgnoreViolationsSection` 已实现，补全9个缺失 i18n 键（`sandbox.ignoreViolations*`）
- i18n：en.json + zh-CN.json 同步添加所有新键

### Iteration 681 — 插件 UI i18n + 记忆类型过滤 i18n + TaskOutputCard
- `SettingsPlugins.tsx`（已有完整实现）：补全 `settings.plugins` / `settings.pluginsEmpty` i18n 键
- `MemoryPanel.tsx`：MemdirTab 类型过滤 chip 已有 UI，将硬编码标签替换为 i18n 键（`memory.filterAll/filterUser/filterFeedback/filterProject/filterReference`）
- 新增 `TaskOutputCard.tsx`：indigo 主题、task_id + block 字段展示、JSON 结果走 `TaskDashboardCard`、纯文本走300字符预览+展开；`ToolUseBlock` 接入 `'TaskOutput'`/`'task_output'` 路由
